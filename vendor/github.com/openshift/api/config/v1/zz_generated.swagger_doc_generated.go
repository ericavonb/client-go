...
var map_AuthenticationSpec = map[string]string{
	...
	"oauthMetadata":              "oauthMetadata contains the discovery endpoint data for OAuth 2.0 Authorization Server Metadata for an external OAuth server. This discovery document can be viewed from its served location: oc get --raw '/.well-known/oauth-authorization-server' For further details, see the IETF Draft: https://tools.ietf.org/html/draft-ietf-oauth-discovery-04#section-2 If oauthMetadata.name is non-empty, this value is used to calculate the observed value stored in status.oauthMetadata. Otherwise, observed cluster state may be used to populate status.oauthMetadata. The key \"oauthMetadata\" is used to locate the data. If specified and the config map or expected key is not found, no metadata is served. If the specified metadata is not valid, no metadata is served.",
	"webhookTokenAuthenticators": "webhookTokenAuthenticators configures remote token reviewers. These remote authentication webhooks can be used to verify bearer tokens via the tokenreviews.authentication.k8s.io REST API.  This is required to honor bearer tokens that are provisioned by an external authentication service. If this list is non-empty, it is used to calculate the observed value stored in status.webhookTokenAuthenticators. Otherwise, observed cluster state may be used to populate status.webhookTokenAuthenticators.",
}
...
var map_AuthenticationStatus = map[string]string{
	"type":                       "type identifies the current user facing authentication mode in use. See authentication spec.type.",
	"oauthMetadata":              "oauthMetadata contains the discovery endpoint data for OAuth 2.0 Authorization Server Metadata for an external OAuth server. See authentication spec.oauthMetadata. This contains the observed value based on cluster state or an explicitly set value in spec.oauthMetadata. The namespace for this config map is openshift-config-managed.",
	"webhookTokenAuthenticators": "webhookTokenAuthenticators identifies the current remote token reviewers. See authentication spec.webhookTokenAuthenticators. This contains the observed value based on cluster state or an explicitly set value in spec.webhookTokenAuthenticators. The namespace for referenced config maps is openshift-config-managed.",
}
...
var map_GitHubIdentityProvider = map[string]string{
	...
	"clientSecret":  "clientSecret is a required reference to the secret by name containing the oauth client secret. The key \"clientSecret\" is used to locate the data. If the secret or expected key is not found, the identity provider is not honored.",
...
var map_GitLabIdentityProvider = map[string]string{
	...
	"ca":           "ca is an optional reference to a config map by name containing the PEM-encoded CA bundle. It is used as a trust anchor to validate the TLS certificate presented by the remote server. The key \"ca.crt\" is used to locate the data. If specified and the config map or expected key is not found, the identity provider is not honored. If the specified ca data is not valid, the identity provider is not honored. If empty, the default system roots are used.",
}
...
var map_LDAPIdentityProvider = map[string]string{
	...
	"bindPassword": "bindPassword is an optional reference to a secret by name containing a password to bind with during the search phase. The key \"bindPassword\" is used to locate the data. If specified and the secret or expected key is not found, the identity provider is not honored.",
